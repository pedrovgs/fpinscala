...
object MyModule {
  ...
  def fib(n: Int): Int = {
    @tailrec
    def go(n: Int, previous: Int = 0, next: Int = 1): Int = n match {
      case 0 => previous
      case 1 => next
...
object PolymorphicFunctions {
  ...
  def isSorted[A](as: Array[A], ordered: (A, A) => Boolean): Boolean = {
    @tailrec
    def go(array: Array[A], current: A): Boolean =
      if (array.isEmpty) true
      else if (array.length == 1) ordered(current, array(0))
      else if (!ordered(current, array(0))) false
      else go(array.tail, array.head)
